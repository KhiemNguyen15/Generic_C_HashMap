...
void listRemoveRange(ArrayList **listPtr, const size_t start,
                     const size_t end) {
  ArrayList *list = *listPtr;
...
size_t listSize(const ArrayList *list) { return list->size; }
...
void reallocList(ArrayList **listPtr, const bool isUp) {
  ...
  if (isUp) {
    list->arr = (Data *)realloc(list->arr, list->capacity * INITIAL_CAPACITY *
                                               sizeof(Data));
    list->capacity *= INITIAL_CAPACITY;
  } else {
    list->arr = (Data *)realloc(list->arr, list->capacity / INITIAL_CAPACITY *
