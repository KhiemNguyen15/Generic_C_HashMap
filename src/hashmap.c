...
{
    HashMap *newMap = createMap(toClone->size, toClone->convert, toClone->comparePairKeys);
...
void mapPutAll(HashMap **mapPtr, const HashMap *toPut)
{
    ...
    {
        mapPut(&map, listGet(toPut->keys, i), mapGet(toPut, listGet(toPut->keys, i)));
    }
}
